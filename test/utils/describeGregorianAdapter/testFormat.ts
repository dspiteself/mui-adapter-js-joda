/**
 * Ported from MUI X `test/utils/pickers/describeGregorianAdapter/testFormat.ts` (MIT).
 */
...
const expectedWeekdayShortFormat = (adapterLib: string) => {
  switch (adapterLib) {
    ...
    case 'moment':
    case 'js-joda':
      // js-joda's `EEE` pattern produces the abbreviated weekday ("Wed"),
      // matching Moment's `dd` rather than Day.js's 2-letter form.
      return 'Wed';
    default:
      return 'We';
  }
