/**
 * Ported verbatim from MUI X `test/utils/pickers/describeGregorianAdapter/testCalculations.ts` (MIT).
 * Modifications: chai → vitest via `expect-chai-compat`, local helper imports.
 */
...
export const testCalculations: DescribeGregorianAdapterTestSuite = ({
  ...
  describe('Method: date', () => {
    ...
    it('should parse locale strings', () => {
      if (adapter.isTimezoneCompatible) {
        const test = (timezone: PickersTimezone) => {
          [adapterTZ, adapterFr].forEach((instance) => {
            const dateWithZone = instance.date(
              TEST_DATE_LOCALE_STRING,
              timezone,
            );
            expect(instance.getTimezone(dateWithZone)).to.equal(timezone);
            expect(instance.format(dateWithZone, 'fullTime24h')).to.equal(
              '00:00',
            );
          });
        };
    ...
    it.skipIf(adapterTZ.lib !== 'dayjs')('should parse undefined', () => {
      if (adapter.isTimezoneCompatible) {
        const testTodayZone = (timezone: PickersTimezone) => {
          ...
          expect(
            Math.abs(adapterTZ.toJsDate(dateWithZone).getTime() - Date.now()),
          ).to.be.lessThan(5);
        };
    ...
    it('should work without args', () => {
      // Upstream uses `adapter.date().valueOf()`; js-joda's `valueOf()` is not a
      // numeric timestamp, so we go via `toJsDate` which every adapter exposes.
      const date = adapter.toJsDate(adapter.date()!).getTime();
  ...
  it.skipIf(!adapter.isTimezoneCompatible)(
    ...
    () => {
      const dateWithZone = adapter.date(
        '2023-10-30T11:44:00.000Z',
        'Europe/London',
      );
  ...
  it('Method: setTimezone', () => {
    if (adapter.isTimezoneCompatible) {
      const test = (timezone: PickersTimezone) => {
        ...
        const dateWithDefaultTimezone = adapter.setTimezone(
          dateWithLocaleTimezone,
          'default',
        );
  ...
  it('Method: parse', () => {
    expect(
      adapter.parse('10/30/2018 11:44', adapter.formats.keyboardDateTime24h),
    ).toEqualDateTime('2018-10-30T11:44:00.000Z');

    expect(
      adapter.parse('10/30/2018', adapter.formats.keyboardDate),
    ).toEqualDateTime('2018-10-30T00:00:00.000Z');

    expect(adapter.parse('', adapter.formats.keyboardDate)).to.equal(null);

    expect(
      adapter.isValid(adapter.parse('99/99/9999', adapter.formats.keyboardDate)),
    ).to.equal(false);
  });
